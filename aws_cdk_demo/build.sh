...
# Prerequisite:  Run `aws configure`
...
# rm source.bat # windows only

# install requirements within venv
# pip install -r requirements.txt

# install requirements-dev within venv to enable pytesting
# pip install -r requirements-dev.txt
# deactivate && source .venv/bin/activate

# run tests with `pytest`

# mkdir -p lib/blueprints
# cdk synth > lib/blueprints/source_simple_stack.yaml
# cdk bootstrap
# cdk deploy
...
#     echo "Also provide the docker host dir."
#     echo "For example: ./build-s3-dist.sh solutions trademarked-solution-name v1.0.0 /Users/<username>/projects/aws-demo"
#     exit 1
# fi
...
# echo 'docker run --entrypoint /bin/bash -v $4/source/lib/blueprints/byom/lambdas/sagemaker_layer:/var/task public.ecr.aws/lambda/python:3.9 -c "cat requirements.txt; pip3 install -r requirements.txt -t ./python; exit"'
# docker run --entrypoint /bin/bash -v $4/source/lib/blueprints/byom/lambdas/sagemaker_layer:/var/task public.ecr.aws/lambda/python:3.9 -c "cat requirements.txt; pip3 install -r requirements.txt -t ./python; exit"
